...
def discover_dashboard_csv_groups(
    ...
    force_refresh: bool = False,
    window_minutes: int = DEFAULT_DASHBOARD_GROUP_WINDOW_MINUTES,
    site_prefix: str = JBP19_SITE_PREFIX,
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "JBP19 report and qcli operations helper. With no inputs, batch-process pending "
            "qcli jbp19 *_full_report.xlsx files and JBP19 dashboard Rack All CSV groups "
    ...
    parser.add_argument(
        "--print-qcli-command",
        action="store_true",
        help="Print JBP19 qcli hc-summary command(s) for --targets, then exit.",
    )
    parser.add_argument(
        "--targets",
        dest="qcli_targets",
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Print latest JBP19 count summaries. Counts are not printed by default. "
            "Examples: jbp19, 19."
        ),
    )
    parser.add_argument(
    ...
    try:
        if args.print_qcli_command:
            commands = jbp19_qcli_commands_text(
                args.qcli_targets,
                "",
        ...
        if args.count_summary_tag.strip():
            ...
            if not requested_prefixes or any(prefix != batch_site_prefix for prefix in requested_prefixes):
                raise ValueError(f"This script is for {batch_site_prefix.upper()} only; use the JBP15 script for JBP15.")
        else:
