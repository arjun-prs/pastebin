...
commands_strings = ["show chassis hardware | match Chassis"]

# Regex pattern to match any alphanumeric serial number format before the next whitespace
serial_number_pattern = r'\b\w+\b'
...
def authenticate_host(hostname):
    try:
        ...
        child = pexpect.spawn(f"ssh {hostname}", env={'TERM': 'dumb'})  # Increased timeout
        child.expect("assword:")
        time.sleep(0.5)
        child.sendline(password)
        child.expect(['>', '#'])  # Handle different prompt types
        time.sleep(0.5)
        return child
    except pexpect.exceptions.TIMEOUT:
        log(f"Error authenticating {hostname}: Timeout exceeded.")
        return None


def execute_command(child, command):
    # Send the command to the device explicitly with a newline character
    # print(f"Sending command: {command}")  # Debugging log to ensure command format
    child.sendline(command + "\n")  # Explicitly adding a newline character
    time.sleep(1)  # Allow some time for the command to execute

    # Expect any of the common prompt characters after command execution
    child.expect(['>', '#', '{master:0}'], timeout=10)
    result = child.before.decode('utf-8', 'ignore')

    # Debug: Log the full command output to see exactly what's returned
    # print("Full command output:")
    # print(result)

    lines = result.splitlines()
    for line in lines:
        if "Chassis" in line:  # Look for the line that contains the relevant data
            serial_number_match = re.search(r'\b\w+\s+([A-Z0-9]+)\b',
                                            line.strip())  # Improved regex to extract the serial number
            if serial_number_match:
                serial_number = serial_number_match.group(1)  # Extracting the serial number
                #print(f"Extracted serial number: {serial_number}")
                return serial_number  # Return the matched serial number

    print("Serial number could not be extracted.")  # Log if no serial number found
    return None
...
def collect_serial_number_for_host(hostname):
    try:
        ...
        if child:
            ...
            return f"Error: Could not authenticate to {hostname}"
    except Exception as e:
...
def process_serial_numbers_from_file(cutsheet_filename):
    ...
    with open(host_serial_filename, "r") as file:
        ...
        for line in lines:
            if ':' in line:  # Check if the line contains both hostname and serial number
                ...
                print(f"Invalid line format in host_serial.txt: {line.strip()}")
